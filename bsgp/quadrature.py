...
def mvnquad(func, means, covs, H: int, Din: int=None, Dout=None):
    ...
    if Din is None:
        raise Exception("If `Din` is passed as `None`, `means` must have a known shape. "
    ...
    if any([d is None for d in Dout]):
        raise Exception("If `Dout` is passed as `None`, the output of `func` must have known "
...
def ndiag_mc(funcs, S: int, Fmu, Fvar, logspace: bool=False, epsilon=None, **Ys):
    ...
    if epsilon is None:
        epsilon = tf.random_normal((S, N, D), dtype=tf.dtypes.float32)
    ...
    def eval_func(func):
        ...
        if logspace:
            log_S = tf.log(tf.cast(S, tf.dtypes.float32))
